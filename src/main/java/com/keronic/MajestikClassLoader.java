...
import java.lang.classfile.FieldBuilder; // Added import
import java.lang.classfile.FieldModel; // Added import
import java.lang.classfile.Annotation; // Added import
import java.lang.classfile.AnnotationElement; // Added import
import java.lang.classfile.AnnotationValue; // Added import
import java.lang.classfile.Interfaces; // Added import
import java.lang.classfile.Superclass; // Added import
import java.lang.classfile.attribute.ConstantValueAttribute; // Added import
import java.lang.classfile.attribute.EnclosingMethodAttribute; // Added import
import java.lang.classfile.attribute.InnerClassesAttribute;
import java.lang.classfile.attribute.InnerClassInfo; // Added import
import java.lang.classfile.attribute.NestHostAttribute; // Added import
import java.lang.classfile.attribute.NestMembersAttribute; // Added import
import java.lang.classfile.attribute.PermittedSubclassesAttribute; // Added import
import java.lang.classfile.attribute.RuntimeInvisibleAnnotationsAttribute; // Added import
import java.lang.classfile.attribute.RuntimeVisibleAnnotationsAttribute; // Added import
import java.lang.classfile.attribute.SignatureAttribute; // Added import
import java.lang.classfile.constantpool.ConstantPoolBuilder;
import java.lang.classfile.constantpool.ConstantValueEntry; // Added import
import java.lang.classfile.constantpool.ClassEntry;
import java.lang.classfile.constantpool.NameAndTypeEntry; // Added import
import java.lang.classfile.constantpool.Utf8Entry;
import java.util.ArrayList;
import java.util.function.BiPredicate; // For listEquals helper
import java.util.List;
import java.lang.classfile.MethodTransform;
import java.lang.classfile.CodeTransform;
import java.lang.classfile.FieldTransform; // Added
import java.lang.classfile.attribute.ExceptionsAttribute;
import java.lang.classfile.instruction.InvokeDynamicInstruction; // Added
import java.lang.constant.DynamicCallSiteDesc; // Added
...
import java.lang.constant.MethodHandleDesc; // Added
...
public class MajestikClassLoader extends ClassLoader {
  ...
  protected Class<?> findClass(String name) throws ClassNotFoundException {
    if (name.startsWith("magik") || name.startsWith("majestik")) {
      try {
        ...
        if (result == null || result.getClassBytes() == null) { // Or check if it's the dummy byte array if we made one
             throw new ClassNotFoundException(name + " (Majestik: loadClassData returned null or empty bytes)");
        }
        return super.defineClass(result.getRemappedClassName(), result.getClassBytes(), 0, result.getClassBytes().length);
      } catch (IOException e) {
  ...
  private ClassLoadingResult loadClassData(String className) throws IOException {
    ...
    byte[] transformedBytes = ClassFile.of().build(remappedThisClassDesc, classBuilder -> {
        ...
        for (ClassElement ce : classModel) { // classModel is iterable
            if (ce instanceof ClassFileVersion) {
                ...
                if (sc.superclassEntry() != null) {
                     ...
                     if (remappedSuperDesc != null && !remappedSuperDesc.equals(sc.superclassEntry().asSymbol())) {
                ...
                List<ClassEntry> remappedInterfaces = ins.interfaces().stream()
                    .map(ifaceEntry -> classBuilder.constantPool().classEntry(mapTypeDesc(ifaceEntry.asSymbol())))
                    .toList();
                boolean interfacesChanged = !listEquals(ins.interfaces(), remappedInterfaces,
                ...
                Utf8Entry remappedSignatureEntry = mapSignatureString(originalSignatureEntry, classBuilder.constantPool());
                ...
                List<InnerClassInfo> remappedInnerClasses = ica.classes().stream()
                    .map(ici -> {
                        ClassDesc originalInner = ici.innerClass().asSymbol();
                        Optional<ClassDesc> originalOuter = ici.outerClass().map(ClassEntry::asSymbol);
                        ...
                        Optional<ClassDesc> remappedOuter = originalOuter.map(this::mapTypeDesc);

                        if (originalInner != remappedInner ||
                            !originalOuter.equals(remappedOuter) ) {
                             Optional<ClassEntry> remappedOuterEntry = remappedOuter.map(cd -> classBuilder.constantPool().classEntry(cd));
                             return InnerClassInfo.of(classBuilder.constantPool().classEntry(remappedInner),
                                                      remappedOuterEntry,
                                                      originalInnerName,
                                                      flags);
                        }
                ...
                boolean innerClassesChanged = !listEquals(ica.classes(), remappedInnerClasses, (o, r) -> o == r);
                ...
                ClassDesc remappedEnclosingClass = mapTypeDesc(ema.enclosingClass().asSymbol());
                Optional<NameAndTypeEntry> originalMethodNat = ema.enclosingMethod();
                Optional<NameAndTypeEntry> remappedMethodNat = originalMethodNat.map(nat -> {
                    MethodTypeDesc originalTypeDesc = MethodTypeDesc.ofDescriptor(nat.type().stringValue());
                    MethodTypeDesc remappedDesc = mapMethodDesc(originalTypeDesc);
                    // If type changed, create new NameAndTypeEntry
                    return originalTypeDesc == remappedDesc ? nat : classBuilder.constantPool().nameAndTypeEntry(nat.name().stringValue(), remappedDesc);
                });

                if (!remappedEnclosingClass.equals(ema.enclosingClass().asSymbol()) || !originalMethodNat.equals(remappedMethodNat)) {
                     classBuilder.with(EnclosingMethodAttribute.of(classBuilder.constantPool().classEntry(remappedEnclosingClass), remappedMethodNat));
                } else {
                    classBuilder.with(ema);
                }
            } else if (ce instanceof RuntimeVisibleAnnotationsAttribute rva) {
                List<Annotation> remappedAnnotations = mapAnnotations(rva.annotations(), classBuilder.constantPool());
                if (!listEquals(rva.annotations(), remappedAnnotations, (o,r) -> o==r)) {
                    classBuilder.with(RuntimeVisibleAnnotationsAttribute.of(remappedAnnotations));
                } else {
                    classBuilder.with(rva);
                }
            } else if (ce instanceof RuntimeInvisibleAnnotationsAttribute ria) {
                List<Annotation> remappedAnnotations = mapAnnotations(ria.annotations(), classBuilder.constantPool());
                 if (!listEquals(ria.annotations(), remappedAnnotations, (o,r) -> o==r)) {
                    classBuilder.with(RuntimeInvisibleAnnotationsAttribute.of(remappedAnnotations));
                } else {
                ...
                if (!remappedHost.equals(nha.nestHost().asSymbol())) {
                    classBuilder.with(NestHostAttribute.of(classBuilder.constantPool().classEntry(remappedHost)));
                } else {
                    classBuilder.with(nha);
                }
            } else if (ce instanceof NestMembersAttribute nma) {
                List<ClassEntry> remappedMembers = nma.nestMembers().stream()
                    .map(memberEntry -> classBuilder.constantPool().classEntry(mapTypeDesc(memberEntry.asSymbol())))
                    .toList();
                if (!listEquals(nma.nestMembers(), remappedMembers, (o,r) -> o.asSymbol().equals(r.asSymbol()))) {
                ...
                List<ClassEntry> remappedSubclasses = psa.permittedSubclasses().stream()
                    .map(subclassEntry -> classBuilder.constantPool().classEntry(mapTypeDesc(subclassEntry.asSymbol())))
                    .toList();
                 if (!listEquals(psa.permittedSubclasses(), remappedSubclasses, (o,r) -> o.asSymbol().equals(r.asSymbol()))) {
                ...
                classBuilder.withMethod(mm.methodName().stringValue(),
                                       ...
                                       mb -> mb.transform(mm, createMethodRemappingTransform(mb.constantPool())));
            } else if (ce instanceof FieldModel fm) {
                classBuilder.withField(fm.fieldName().stringValue(),
    ...
    if (remappedThisClassDesc.isArray()) {
        ...
        // Ensure internal slashes are dots for defineClass if that's the expectation for array component FQNs.
  ...
  private MethodTypeDesc mapMethodDesc(MethodTypeDesc desc) {
      ...
      List<ClassDesc> remappedParameterTypes = desc.parameterList().stream()
                                                   .map(this::mapTypeDesc)
                                                   .toList();
      boolean changed = !remappedReturnType.equals(desc.returnType()) ||
                        !remappedParameterTypes.equals(desc.parameterList());

      if (changed) {
          return MethodTypeDesc.of(remappedReturnType, remappedParameterTypes.toArray(new ClassDesc[0]));
      } else {
          return desc;
      }
  }
    ...
    return (cob, coe) -> { // cob is CodeBuilder, coe is CodeElement
        switch (coe) {
            ...
            case NewMultiArrayInstruction instr:
                ...
                break;
            case LoadConstantInstruction ldcInstr:
            ...
            case InvokeDynamicInstruction idi: {
                ...
                DirectMethodHandleDesc remappedBootstrapMethod = mapDirectMethodHandle(originalBootstrapMethod, cob.constantPool());
                List<ConstantDesc> remappedBootstrapArgs = originalBootstrapArgs.stream()
                ...
                boolean bootstrapArgsChanged = !listEquals(originalBootstrapArgs, remappedBootstrapArgs, (o, r) -> o == r);
    ...
    return (methodBuilder, methodElement) -> {
        switch (methodElement) {
            ...
            case ExceptionsAttribute ea: {
                List<ClassEntry> remappedExceptions = ea.exceptions().stream()
                    .map(e -> methodCp.classEntry(mapTypeDesc(e.asSymbol())))
                    .toList();
                boolean changed = ea.exceptions().size() != remappedExceptions.size() ||
                                  !listEquals(ea.exceptions(), remappedExceptions, (o,r) -> o.asSymbol().equals(r.asSymbol()));
                if (changed) {
  ...
  private ConstantDesc mapConstantDesc(ConstantDesc constantDesc, ConstantPoolBuilder cp) {
      ...
      return switch (constantDesc) {
          ...
          case String originalString -> {
                ...
                String tempRemappedString = originalString.replace("com.gesmallworld.magik", "com.keronic.majestik");
                if (!originalString.equals(tempRemappedString)) {
                    remappedString = tempRemappedString;
                } else {
                    // Try slash form only if dot form didn't match
                    tempRemappedString = originalString.replace("com/gesmallworld/magik", "com/keronic/majestik");
                    if (!originalString.equals(tempRemappedString)) {
                        remappedString = tempRemappedString;
                    }
                }
  ...
  private DirectMethodHandleDesc mapDirectMethodHandle(DirectMethodHandleDesc dmhd, ConstantPoolBuilder cp) {
      if (dmhd == null) {
          return null;
      }
      ...
      switch (dmhd.kind()) {
          case GETTER, SETTER, STATIC_GETTER, STATIC_SETTER:
              ...
              if (ownerChanged || fieldType != remappedFieldType) {
                  return MethodHandleDesc.ofField(dmhd.kind(), remappedOwner, methodName, remappedFieldType);
              }
              break;
          default:
              ...
              if (ownerChanged || methodType != remappedMethodType) {
                  return MethodHandleDesc.ofMethod(dmhd.kind(), remappedOwner, methodName, remappedMethodType);
              }
              break;
      }
      return dmhd;
  ...
  private DynamicConstantDesc<?> mapDynamicConstantDesc(DynamicConstantDesc<?> dcd, ConstantPoolBuilder cp) {
      if (dcd == null) {
          return null;
      }

      DirectMethodHandleDesc originalBootstrapMethod = dcd.bootstrapMethod();
      DirectMethodHandleDesc remappedBootstrapMethod = mapDirectMethodHandle(originalBootstrapMethod, cp);
      ...
      List<ConstantDesc> remappedBootstrapArgs = originalBootstrapArgs.stream()
          .map(arg -> mapConstantDesc(arg, cp))
          .toList();
      ...
      if (bootstrapMethodChanged || constantTypeChanged || bootstrapArgsChanged) {
          return DynamicConstantDesc.ofNamed(remappedBootstrapMethod,
                                           constantName,
  ...
  // Helper method for mapAnnotations, assuming it's similar to how other list transformations are handled
  private List<Annotation> mapAnnotations(List<Annotation> originalAnnotations, ConstantPoolBuilder cp) {
      List<Annotation> remappedAnnotations = originalAnnotations.stream()
                                               .map(ann -> mapAnnotation(ann, cp))
                                               .toList();
      // Determine if any annotation actually changed.
      boolean changed = false;
      for (int i = 0; i < originalAnnotations.size(); i++) {
          if (originalAnnotations.get(i) != remappedAnnotations.get(i)) { // Relies on mapAnnotation returning original if no change
              changed = true;
              break;
          }
      }
  ...
  private AnnotationValue mapAnnotationValue(AnnotationValue val, ConstantPoolBuilder cp) {
      ...
      return switch (val) {
          ...
          case AnnotationValue.OfArray arr -> {
              List<AnnotationValue> remappedValues = arr.values().stream()
                                                         .map(v -> mapAnnotationValue(v, cp))
                                                         .toList();
              ...
              if (remappedValues.size() != arr.values().size()) { // Should not happen with toList()
                  ...
                  for (int i = 0; i < remappedValues.size(); i++) {
                      if (remappedValues.get(i) != arr.values().get(i)) { // Reference check, assumes mapAnnotationValue returns original if no change
                          changed = true;
                          break;
                      }
                  }
          ...
          case AnnotationValue.OfClass oc -> {
              ...
              yield originalClassDesc == remappedClassDesc ? oc : AnnotationValue.ofClass(remappedClassDesc);
          }
          case AnnotationValue.OfEnum oe -> {
              ...
              yield originalEnumClassDesc == remappedEnumClassDesc ? oe : AnnotationValue.ofEnum(remappedEnumClassDesc, oe.constantName().stringValue());
          }
  ...
  private Annotation mapAnnotation(Annotation ann, ConstantPoolBuilder cp) {
      ...
      List<AnnotationElement> remappedElements = originalElements.stream()
          .map(el -> {
              ...
              return originalValue == remappedValue ? el : AnnotationElement.of(el.name(), remappedValue);
          })
          .toList();
  ...
  private Utf8Entry mapSignatureString(Utf8Entry signatureUtf8Entry, ConstantPoolBuilder cp) {
      ...
      remappedSignature = remappedSignature.replace("Lcom/gesmallworld/magik/", "Lcom/keronic/majestik/");
