/*
 * Copyright (c) 2023, 2024, Oracle and/or its affiliates. All rights reserved. DO NOT ALTER OR
 ...
 */
package com.keronic.majestik.internal.impl;
...
public record ClassRemapper(Function<ClassDesc, ClassDesc> mapFunction) implements ClassTransform {
  /**
   * Creates new instance of {@code ClassRemapper} instructed with a map function. Map function must
   * return valid {@link java.lang.constant.ClassDesc} of an interface or a class, even for identity
   * mappings.
   *
  ...
  public static ClassRemapper of(Function<ClassDesc, ClassDesc> mapFunction) {
    return new ClassRemapper(Objects.requireNonNull(mapFunction));
  }
    ...
    switch (cle) {
      ...
      case MethodModel mm ->
          clb.withMethod(
              ...
              mm.flags().flagsMask(),
              mb -> mb.transform(mm, asMethodTransform()));
      case Superclass sc -> clb.withSuperclass(map(sc.superclassEntry().asSymbol()));
      case Interfaces ins ->
          clb.withInterfaceSymbols(mappedList(ins.interfaces(), in -> map(in.asSymbol())));
      ...
      case EnclosingMethodAttribute ema ->
          clb.with(
              EnclosingMethodAttribute.of(
                  map(ema.enclosingClass().asSymbol()),
      ...
      case ModuleAttribute ma ->
          clb.with(
              ModuleAttribute.of(
                  ...
                  ma.opens(),
                  ma.uses().stream()
                      .map(ce -> clb.constantPool().classEntry(map(ce.asSymbol())))
                      .toList(),
                  ma.provides().stream()
                      .map(
                          mp ->
                              ModuleProvideInfo.of(
                                  map(mp.provides().asSymbol()),
                                  mp.providesWith().stream()
                                      .map(pw -> map(pw.asSymbol()))
                                      .toList()))
                      .toList()));
      ...
      case NestMembersAttribute nma ->
          clb.with(
              NestMembersAttribute.ofSymbols(
                  nma.nestMembers().stream().map(nm -> map(nm.asSymbol())).toList()));
    ...
    return (MethodBuilder mb, MethodElement me) -> {
      switch (me) {
        case AnnotationDefaultAttribute ada ->
            mb.with(AnnotationDefaultAttribute.of(mapAnnotationValue(ada.defaultValue())));
        case CodeModel com -> mb.transformCode(com, asCodeTransform());
        case ExceptionsAttribute ea ->
            mb.with(
                ExceptionsAttribute.ofSymbols(
                    ea.exceptions().stream().map(ce -> map(ce.asSymbol())).toList()));
        case SignatureAttribute sa ->
    ...
    return (CodeBuilder cob, CodeElement coe) -> {
      switch (coe) {
        case FieldInstruction fai ->
            cob.fieldAccess(
                fai.opcode(),
                map(fai.owner().asSymbol()),
                fai.name().stringValue(),
                map(fai.typeSymbol()));
        case InvokeInstruction ii ->
            cob.invoke(
                ii.opcode(),
                map(ii.owner().asSymbol()),
                ii.name().stringValue(),
                mapMethodDesc(ii.typeSymbol()),
                ii.isInterface());
        case InvokeDynamicInstruction idi ->
            cob.invokedynamic(
                DynamicCallSiteDesc.of(
                    mapDirectMethodHandle(idi.bootstrapMethod()),
                    idi.name().stringValue(),
                    mapMethodDesc(idi.typeSymbol()),
                    idi.bootstrapArgs().stream()
        ...
        case ExceptionCatch c ->
            cob.exceptionCatch(
                ...
                c.tryEnd(),
                c.handler(),
                c.catchType().map(d -> cob.constantPool().classEntry(map(d.asSymbol()))));
        ...
        case LocalVariableType c ->
            cob.localVariableType(
                c.slot(),
                c.name().stringValue(),
                mapSignature(c.signatureSymbol()),
                c.startScope(),
                c.endScope());
  ...
  Annotation mapAnnotation(Annotation a) {
    return Annotation.of(
        map(a.classSymbol()),
        a.elements().stream()
            .map(el -> AnnotationElement.of(el.name(), mapAnnotationValue(el.value())))
            .toList());
  }
  ...
  List<TypeAnnotation> mapTypeAnnotations(List<TypeAnnotation> typeAnnotations) {
    return typeAnnotations.stream()
        .map(a -> TypeAnnotation.of(a.targetInfo(), a.targetPath(), mapAnnotation(a.annotation())))
        .toList();
  }
